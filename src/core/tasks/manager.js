...
async function listTasks(projectIdOrOptions, maybeOptions) {
  ...
  const state = await settings.taskStateStore.readTaskState(settings.taskStatePath);
  const tasks = listTaskStates(state).filter((task) => !projectId || task.projectId === projectId);
  return tasks.map((task) => enrichTask(task, tasks));
}
...
async function inspectTask(taskId, options = {}) {
  ...
  return enrichTask(task, projectTasks);
}
...
async function createTask(input, options = {}) {
  ...
  return withProjectLock(projectId, async () => {
    ...
    const [projectState, instanceState] = await Promise.all([
    ...
    return enrichFromState(getTaskState(updated, taskId), updated);
  });
}
...
async function mutateTask(taskId, options, mutation) {
  ...
  return withProjectLock(initialTask.projectId, async () => {
    const settings = resolveSettings(options);
    const projectState = await settings.projectStateStore.readProjectState(settings.projectStatePath);
...
function enrichFromState(task, state) {
  const tasks = listTaskStates(state).filter((item) => item.projectId === task.projectId);
  return enrichTask(task, tasks);
}
function enrichTask(task, projectTasks) {
  const blocking = calculateTaskBlocking(task, projectTasks);
  return { ...task, computedStatus: blocking.status, dependencyIssues: blocking.issues };
}
